...
PROGBLEM_INPUT_TXT = Path("/Users/pergrapatin/Source/AOC2017/src/"\
    +"day18-Duet/input.txt").read_text()
...
class AssemblyProcessorProblemB:
    """Class for Advent Of Code assembly language processing
    """
    def __init__(self, code_array, value_for_p):
        self.registry_dict = { 'p' : value_for_p
        }
        self.program_counter = 0
        self._code_array = code_array
        self.partner_program = None
        self.input_queue = []
        self.send_counter = 0

    def set_partner_program(self, partner_program):
        """Sets the partner program that will be used to send and receive
        """
        self.partner_program = partner_program

    def _snd(self, code_string):
        """snd X
        send X to partnerprogram
        """
        parts = code_string.split()
        x_part = parts[1]
        partner_program = self.partner_program
        partner_program.input_queue.append(self.registry_dict[x_part])
    ...
    def _rcv(self, code_string):
        """_rcv X
        receive value and store in X
        """
        parts = code_string.split()
        x_part = parts[1]
        if len(self.input_queue) > 0:
            self.program_counter += 1
            self.registry_dict[x_part] = self.input_queue.pop(0)
        else:
            pass #Nothing in queue wait here
    def _jgz(self, code_string):
        """jgz X Y
        jumps with an offset of the value of Y, but only if the value of X is greater
        than zero. (An offset of 2 skips the next instruction, an offset of -1 jumps
...
def problem_b(input_string, expected_result):
    ...
    """
    code_strings = string_worker(input_string)
    runner_instance_0 = AssemblyProcessorProblemB(code_strings, 0)
    runner_instance_1 = AssemblyProcessorProblemB(code_strings, 1)
    runner_instance_1.partner_program = runner_instance_0
    runner_instance_0.partner_program = runner_instance_1

    previous_pc0 = -1
    previous_pc1 = -1
    for _ in range(1000):
        while previous_pc1 != runner_instance_1.program_counter:
            previous_pc1 = runner_instance_1.program_counter
            runner_instance_1.execute()
        while previous_pc0 != runner_instance_0.program_counter:
            previous_pc0 = runner_instance_0.program_counter
            runner_instance_0.execute()
        previous_pc0 = -1
        previous_pc1 = -1

    solution = runner_instance_1.send_counter
    if solution == expected_result:
        print("Correct solution found:", solution)
    else:
        print("Incorrect solution, we got:", solution, "expected:", expected_result)

problem_b(PROGBLEM_INPUT_TXT, 7366)
print("\n")
